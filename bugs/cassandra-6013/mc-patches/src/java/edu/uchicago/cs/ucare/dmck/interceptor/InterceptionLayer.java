...
public class InterceptionLayer {
    ...
    public static void updateWorkloadAccomplishment(long workloadId, boolean isApplied) {
        String filename = "cassWorkloadUpdate-" + workloadId;

        String content = "";
        content += "workloadId=" + workloadId + "\n";
        content += "isApplied=" + isApplied + "\n";

        writePacketToFile(filename, content);

        LOG.info(
                "[DMCK] Intercept a workload accomplishment update: "
                        + " workloadId: "
                        + workloadId
                        + " isApplied: "
                        + isApplied
    ...
    public static void notifyNullPointerException(long sender) {
        String filename = "cassNullPointerException-" + sender;
        ...
        LOG.info(
                "[DMCK] Intercept a NullPointException notification: "
                        + " sender: "
                        + sender
                        + "/"
                        + filename);
    ...
    public static void enableReceiving(int sender, int recv, String verb) {
        ...
        LOG.info("[DMCK] Enable receiving message " + verb + " from node-" + sender + " at node-" + recv);
    }
    ...
    // Sender Sequencer Resetter
    public static void resetSenderSequencer() {
        latestSenderStep = 0;
        senderSequencer = "";
        LOG.info("[DMCK] Sender Sequencer=" + senderSequencer);
    }

    // Receiver Sequencer Resetter
    public static void resetReceiverSequencer() {
        latestReceiverStep = 0;
        receiverSequencer = "";
        LOG.info("[DMCK] Receiver Sequencer=" + receiverSequencer);
    }
    ...
    public static void waitForExecuteCas(long nodeId) {
        File file = new File("executeCas-" + nodeId);
        while (!file.exists()) {
            try {
                Thread.sleep(0, 100);
            } catch (InterruptedException ie) {
        ...
        if (file.exists()) {
            if (file.delete()) {
                LOG.info("executeCas-" + nodeId + " is deleted");
            } else {
                LOG.info("Error in deleting " + "executeCas-" + nodeId);
            }
        }
        String newFilePath = "executeCas-" + (nodeId+1);
        ...
        if (!newFile.exists()) {
            try {
                if (newFile.createNewFile()) {
                    ...
                    LOG.info("Error in creating " + newFilePath);
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
