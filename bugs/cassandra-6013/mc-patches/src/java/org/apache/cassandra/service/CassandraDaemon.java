...
import edu.uchicago.cs.ucare.dmck.interceptor.ResetWatcher;
...
public class CassandraDaemon
{
     ...
     */
        ...
        // DMCK
        String addr = DatabaseDescriptor.getRpcAddress().toString();
        int nodeId = Integer.parseInt(addr.substring(addr.length()-1))-1;
        resetWatcher = new ResetWatcher(nodeId);

        ReadDataWatcher readDataWatcher = new ReadDataWatcher(nodeId);
        Thread readDataWatcherThread = new Thread(readDataWatcher);
        readDataWatcherThread.start();
    }
    ...
    public void start()
    {
        ...
        // DMCK
        Thread resetWatcherThread = new Thread(resetWatcher);
        resetWatcherThread.start();
    }
    ...
    private class ReadDataWatcher implements Runnable {
        ...
        public ReadDataWatcher(long nodeId) {
            this.nodeId = nodeId;
        }

        public void run() {
            logger.info("ReadDataWatcher is waiting for readData-{}", nodeId);

            File file = new File(System.getProperty("user.dir") + "/readData-" + nodeId);
            while (!Thread.interrupted()) {
                if (file.exists()) {
                    logger.info("readData-{} exists", nodeId);
                    String query = "SELECT * FROM test.tests;";
                    try {
                        ...
                        if (file.delete()) {
                            logger.info("readData-{} is deleted", nodeId);
                        } else {
                            logger.info("Error in deleting readData-{}", nodeId);
                        }
                        ...
                        try {
                			// PrintWriter writer = new PrintWriter(System.getProperty("user.dir") + "/readDataResult-" + nodeId, "UTF-8");
                            PrintWriter writer = new PrintWriter(System.getProperty("user.dir") + "/temp-verify", "UTF-8");
                			writer.println(values);
                			writer.close();
                		} catch (Exception e){
                			logger.info("ERROR in writing temp-verify result.");
                			e.printStackTrace();
                		}
                    } catch (Exception e) {
                        e.printStackTrace();
