...
public final class MessagingService implements MessagingServiceMBean {
  ...
  public enum Verb {
    ...
    READ_REQUEST_RESPONSE, // DMCK
    PAXOS_PREPARE_RESPONSE, // DMCK
    PAXOS_PROPOSE_RESPONSE, // DMCK
    PAXOS_COMMIT_RESPONSE, // DMCK
    @Deprecated
    STREAM_INITIATE,
    @Deprecated
    ...
    ECHO,
    REPAIR_MESSAGE,
    // use as padding for backwards compatability where a previous version needs to validate a verb
    // from the future.
    PAXOS_PREPARE,
    PAXOS_PROPOSE,
    PAXOS_COMMIT,
  ...
  public static final EnumMap<MessagingService.Verb, Stage> verbStages =
      new EnumMap<MessagingService.Verb, Stage>(MessagingService.Verb.class) {
        {
          ...
          put(Verb.READ_REQUEST_RESPONSE, Stage.REQUEST_RESPONSE); // DMCK
          put(Verb.PAXOS_PREPARE_RESPONSE, Stage.REQUEST_RESPONSE); // DMCK
          put(Verb.PAXOS_PROPOSE_RESPONSE, Stage.REQUEST_RESPONSE); // DMCK
          put(Verb.PAXOS_COMMIT_RESPONSE, Stage.REQUEST_RESPONSE); // DMCK
          put(Verb.INTERNAL_RESPONSE, Stage.INTERNAL_RESPONSE);

          put(
              Verb.STREAM_REPLY,
              Stage.MISC); // actually handled by FileStreamTask and streamExecutors
   ...
   * exception is the REQUEST_RESPONSE verb, which just means "a reply to something you told me to
   * do." Traditionally, this was fine since each VerbHandler knew what type of payload it expected,
   * and handled the deserialization itself. Now that we do that in ITC, to avoid the extra copy to
   * an intermediary byte[] (See CASSANDRA-3716), we need to wire that up to the CallbackInfo object
   * (see below).
   */
  public static final EnumMap<Verb, IVersionedSerializer<?>> verbSerializers =
      new EnumMap<Verb, IVersionedSerializer<?>>(Verb.class) {
        {
          put(Verb.REQUEST_RESPONSE, CallbackDeterminedSerializer.instance);
          put(Verb.READ_REQUEST_RESPONSE, CallbackDeterminedSerializer.instance); // DMCK
          put(Verb.PAXOS_PREPARE_RESPONSE, CallbackDeterminedSerializer.instance); // DMCK
          put(Verb.PAXOS_PROPOSE_RESPONSE, CallbackDeterminedSerializer.instance); // DMCK
          put(Verb.PAXOS_COMMIT_RESPONSE, CallbackDeterminedSerializer.instance); // DMCK
  ...
  /**
   * A Map of what kind of serializer to wire up to a REQUEST_RESPONSE callback, based on outbound
   * Verb.
   */
  ...
  /**
   * a placeholder class that means "deserialize using the callback." We can't implement this
   * without special-case code in InboundTcpConnection because there is no way to pass the message
   * id to IVersionedSerializer.
   */
   ...
   *
   * <p>See CASSANDRA-3494 for the background. We have streaming in place so we do not want to limit
   * ourselves to one stream at a time for throttling reasons. But, we also do not want to just
   * arbitrarily stream an unlimited amount of files at once because a single destination might have
   * hundreds of files pending and it would cause a seek storm. So, transfer exactly one file per
   * destination host. That puts a very natural rate limit on it, in addition to mapping well to the
   * expected behavior in many cases.
   *
   * <p>We will create our stream executors with a core size of 0 so that they time out and do not
   * consume threads. This means the overhead in the degenerate case of having streamed to everyone
   * in the ring over time as a ring changes, is not going to be a thread per node - but rather an
   * instance per node. That's totally fine.
   */
  private final ConcurrentMap<InetAddress, DebuggableThreadPoolExecutor> streamExecutors =
      new NonBlockingHashMap<InetAddress, DebuggableThreadPoolExecutor>();

  private final NonBlockingHashMap<InetAddress, OutboundTcpConnectionPool> connectionManagers =
   ...
   * all correspond to client requests or something triggered by them; we don't want to drop
   * internal messages like bootstrap or repair notifications.
   */
  public static final EnumSet<Verb> DROPPABLE_VERBS =
      EnumSet.of(
          ...
          Verb.READ_REQUEST_RESPONSE, // DMCK
          Verb.PAXOS_PREPARE_RESPONSE, // DMCK
          Verb.PAXOS_PROPOSE_RESPONSE, // DMCK
          Verb.PAXOS_COMMIT_RESPONSE); // DMCK

  // total dropped message counts for server lifetime
  private final Map<Verb, DroppedMessageMetrics> droppedMessages =
      new EnumMap<Verb, DroppedMessageMetrics>(Verb.class);
  ...
  // DMCK
  public static Map<Long, MessageOut> messageMap = new HashMap<Long, MessageOut>();
  public static Map<Long, Integer> messageIdMap = new HashMap<Long, Integer>();
  public static Map<Long, InetAddress> destinationMap = new HashMap<Long, InetAddress>();
  ...
  private MessagingService() {
    ...
    StorageService.scheduledTasks.scheduleWithFixedDelay(
        logDropped, LOG_DROPPED_INTERVAL_IN_MS, LOG_DROPPED_INTERVAL_IN_MS, TimeUnit.MILLISECONDS);
    ...
    Function<Pair<Integer, ExpiringMap.CacheableObject<CallbackInfo>>, ?> timeoutReporter =
        new Function<Pair<Integer, ExpiringMap.CacheableObject<CallbackInfo>>, Object>() {
          public Object apply(Pair<Integer, ExpiringMap.CacheableObject<CallbackInfo>> pair) {
            ...
            maybeAddLatency(
                expiredCallbackInfo.callback, expiredCallbackInfo.target, pair.right.timeout);
    ...
    callbacks =
        new ExpiringMap<Integer, CallbackInfo>(
            DatabaseDescriptor.getMinRpcTimeout(), timeoutReporter);
   ...
   *
   * @param cb the callback associated with this message -- this lets us know if it's a message type
   *     we're interested in
   * @param address the host that replied to the message
   * @param latency
   */
  public void maybeAddLatency(IAsyncCallback cb, InetAddress address, long latency) {
    if (cb.isLatencyForSnitch()) addLatency(address, latency);
  }

  public void addLatency(InetAddress address, long latency) {
    for (ILatencySubscriber subscriber : subscribers) subscriber.receiveTiming(address, latency);
  }
  ...
  private List<ServerSocket> getServerSocket(InetAddress localEp) throws ConfigurationException {
    ...
    if (DatabaseDescriptor.getServerEncryptionOptions().internode_encryption
        != ServerEncryptionOptions.InternodeEncryption.none) {
      try {
        ss.add(
            SSLFactory.getServerSocket(
  ...
  public int addCallback(IAsyncCallback cb, MessageOut message, InetAddress to, long timeout) {
    ...
    // If HH is enabled and this is a mutation message => store the message to track for potential
    // hints.
    if (DatabaseDescriptor.hintedHandoffEnabled() && message.verb == Verb.MUTATION)
      previous =
          callbacks.put(
              messageId,
              new CallbackInfo(to, cb, message, callbackDeserializers.get(message.verb)),
              timeout);
    else
      previous =
          callbacks.put(
              messageId,
              new CallbackInfo(to, cb, callbackDeserializers.get(message.verb)),
              timeout);
  ...
  /**
   * Send a message to a given endpoint. This method specifies a callback which is invoked with the
   * actual response. Also holds the message (only mutation messages) to determine if it needs to
   * trigger a hint (uses StorageProxy for that).
   *
   * @param message message to be sent.
   * @param to endpoint to which the message needs to be sent
   * @param cb callback interface which is used to pass the responses or suggest that a timeout
   *     occurred to the invoker of the send(). suggest that a timeout occurred to the invoker of
   *     the send().
  ...
  */
  ...
  public void waitForStreaming() throws InterruptedException {
    // this does not prevent new streams from beginning after a drain begins, but since streams are
    // only
    // started in response to explicit operator action (bootstrap/move/repair/etc) that feels like a
    // feature.
  ...
  /**
   * Wait for callbacks and don't allow any more to be created (since they could require writing
   * hints)
   */
  public void shutdown() {
    logger.info("Waiting for messaging service to quiesce");
    // We may need to schedule hints on the mutation stage, so it's erroneous to shut down the
    // mutation stage first
  ...
  public static void validateMagic(int magic) throws IOException {
    if (magic != PROTOCOL_MAGIC) throw new IOException("invalid protocol header");
  }
  ...
  /**
   * @return the last version associated with address, or @param version if this is the first such
   *     version
   */
  ...
  private static class SocketThread extends Thread {
    ...
    private boolean authenticate(Socket socket) {
      return DatabaseDescriptor.getInternodeAuthenticator()
          .authenticate(socket.getInetAddress(), socket.getPort());
    }
  }
  ...
  public Map<String, Integer> getCommandPendingTasks() {
    ...
    for (Map.Entry<InetAddress, OutboundTcpConnectionPool> entry : connectionManagers.entrySet())
      pendingTasks.put(
          entry.getKey().getHostAddress(), entry.getValue().cmdCon.getPendingMessages());
    return pendingTasks;
  }
  ...
  public Map<String, Long> getCommandCompletedTasks() {
    ...
    for (Map.Entry<InetAddress, OutboundTcpConnectionPool> entry : connectionManagers.entrySet())
      completedTasks.put(
          entry.getKey().getHostAddress(), entry.getValue().cmdCon.getCompletedMesssages());
  ...
  public Map<String, Long> getCommandDroppedTasks() {
    ...
    for (Map.Entry<InetAddress, OutboundTcpConnectionPool> entry : connectionManagers.entrySet())
      droppedTasks.put(
          entry.getKey().getHostAddress(), entry.getValue().cmdCon.getDroppedMessages());
    return droppedTasks;
  }
  ...
  public Map<String, Integer> getResponsePendingTasks() {
    ...
    for (Map.Entry<InetAddress, OutboundTcpConnectionPool> entry : connectionManagers.entrySet())
      pendingTasks.put(
          entry.getKey().getHostAddress(), entry.getValue().ackCon.getPendingMessages());
    return pendingTasks;
  }
  ...
  public Map<String, Long> getResponseCompletedTasks() {
    ...
    for (Map.Entry<InetAddress, OutboundTcpConnectionPool> entry : connectionManagers.entrySet())
      completedTasks.put(
          entry.getKey().getHostAddress(), entry.getValue().ackCon.getCompletedMesssages());
    return completedTasks;
  }
