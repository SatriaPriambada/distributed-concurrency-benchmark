    ...
    */
    ...
    public static PrepareResponse prepare(Commit toPrepare)
    {
        synchronized (lockFor(toPrepare.key))
        {
            ...
            {
                logger.info("[DMCK] promising ballot {}", toPrepare.ballot);
            ...
            {
                logger.info("[DMCK] promise rejected; {} is not sufficiently newer than {}", toPrepare, state.inProgressCommit);
    ...
    public static Boolean propose(Commit proposal)
    {
        synchronized (lockFor(proposal.key))
        {
            ...
            {
                logger.info("[DMCK] accepting proposal {}", proposal);
                Tracing.trace("accepting proposal {}", proposal);
                SystemKeyspace.savePaxosProposal(proposal);
                return true;
            }

            logger.info("[DMCK] accept requested for {} but inProgress is now {}", proposal, state.inProgressCommit);
    ...
    public static void commit(Commit proposal)
    {
        ...
        logger.info("[DMCK] committing proposal {}", proposal);
