      ...
      if (GuideModelChecker.class.isAssignableFrom(dmckStrategyClass)) {
        ...
        Constructor<? extends ModelCheckingServerAbstract> guideDMCKConstructor =
            dmckStrategyClass.getConstructor(String.class, FileWatcher.class, Integer.TYPE,
                File.class, String.class, WorkloadDriver.class, String.class);
        dmck = guideDMCKConstructor.newInstance(dmckName, fileWatcher, numNode, pathFile,
            workingDir, workloadDriver, ipcDir);
      } else {
        Constructor<? extends ModelCheckingServerAbstract> dmckConstructor =
            dmckStrategyClass.getConstructor(String.class, FileWatcher.class, Integer.TYPE,
                Integer.TYPE, Integer.TYPE, String.class, WorkloadDriver.class, String.class);
  ...
  private static void startExploration() {
    ...
    try {
      ...
      Runtime.getRuntime().addShutdownHook(new Thread() {
        public void run() {
          dmck.stopEnsemble(/* forceStop= */true);
        }
      });
      ...
      for (; !finishedFlag.exists(); ++testId) {
        ...
        // if waitingFlag exists, then DMCK stops the target system nodes
        dmck.stopEnsemble(/* forceStop= */false);
