...
public abstract class FileWatcher implements Runnable {
  ...
  public void run() {
    ...
    while (!Thread.interrupted()) {
      if (acceptFile && path.listFiles().length > 0) {
        for (File file : path.listFiles()) {
          ...
          try {
            ...
            proceedEachFile(filename, ev);
            removeProceedFile(filename);
          } catch (Exception e) {
            LOG.error("Experienced failure at processing file=" + filename);
            e.printStackTrace();
          }
        }
      }
  ...
  public void enableEvent(PacketSendTransition event) {
    ...
    LOG.debug("Enable event with ID : " + event.getTransitionId());
  }
