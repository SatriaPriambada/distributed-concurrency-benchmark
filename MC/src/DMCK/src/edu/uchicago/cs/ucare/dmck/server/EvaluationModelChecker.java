...
public abstract class EvaluationModelChecker extends ReductionAlgorithmsModelChecker {
  ...
  public EvaluationModelChecker(String dmckName, FileWatcher fileWatcher, int numNode, int numCrash,
      int numReboot, String workingDir, WorkloadDriver workloadDriver, String ipcDir) {
    super(dmckName, fileWatcher, numNode, numCrash, numReboot, workingDir, workloadDriver, ipcDir);
  }
  ...
  protected boolean isPossibleConcurrent(Transition lastTransition, Transition currentTransition) {
    // if both events are message events, then check whether DMCK needs to consider
    // special case of concurrency that is system specific.
    if (lastTransition instanceof PacketSendTransition
      ...
      PacketSendTransition ct = (PacketSendTransition) currentTransition;
      if (lt.isSystemSpecificConcurrentEvent(ct)) {
        return true;
      }
    }
    // transform any abstract crash or reboot event into a real event.
    ...
    if (allBeforeTransitions == null) {
      ...
      // if current transition does not exist in last transition list of dependency,
