...
public class ZKVerifier extends SpecVerifier {
  ...
  public String verificationDetail() {
    ...
    for (int node = 0; node < dmck.numNode; node++) {
      ...
      if (this.dmck.numInitWorkload > 0) {
  ...
  private boolean checkDataConsistency() {
    ...
    for (int i = 0; i < dmck.numNode; i++) {
      ...
      nodesAliveness += dmck.isNodeOnline[i] ? "1" : "0";
    }
    try {
      LOG.debug("Executing checkDataConsistency script.");
      Runtime.getRuntime().exec(
          dmck.workingDirPath + "/checkConsistency.sh " + dmck.numNode + " /foo " + nodesAliveness)
          .waitFor();
      ...
      String[] values = new String[dmck.numNode];
      for (int i = 0; i < dmck.numNode; i++) {
